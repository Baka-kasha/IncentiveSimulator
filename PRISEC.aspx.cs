...
public partial class PRISEC : System.Web.UI.Page
{
    ...
    public void BindGrid()
    {
        ...
        if (desgna == "Admin")
        {
            ...
            string StrQrry = "";
            StrQrry = "";
            StrQrry = "	 SELECT * FROM [EMPLOYEE_Incentive_TAB] where DIVISION != 'AX' order by ROW_ID ";
            string conn = ConfigurationManager.ConnectionStrings["sqlcon"].ConnectionString;
            SqlDataAdapter adpRpt = new SqlDataAdapter(StrQrry, conn);
            adpRpt.Fill(DsStockist, "DsStockist");
            adpRpt.Fill(dtDsStockist);
    ...
    private void ExportGridToExcel()
    {
        ...
        if (sm == desgna)
        {
            StrQrry = " SELECT  [DESIGNATION],[EMP_NAME],[CLIENT_HQ],[NOP_HQ],[PRIMARY_VAL],[SECONDARY_VAL],[SnBPr] as [SnB %] FROM [EMPLOYEE_Incentive_TAB] where SM ='" + Session["FSCODE"] + "' order by ROW_ID;";

        }
        if (rbm == desgna)
        {
            StrQrry = " SELECT [DESIGNATION],[EMP_NAME],[CLIENT_HQ],[NOP_HQ],[PRIMARY_VAL],[SECONDARY_VAL],[SnBPr] as [SnB %] FROM [EMPLOYEE_Incentive_TAB] where RBM ='" + Session["FSCODE"] + "' order by ROW_ID;";

        }
        if (dsm == desgna)
        {
            StrQrry = " SELECT [DESIGNATION],[EMP_NAME],[CLIENT_HQ],[NOP_HQ],[PRIMARY_VAL],[SECONDARY_VAL],[SnBPr] as [SnB %] FROM [EMPLOYEE_Incentive_TAB] where DSM ='" + Session["FSCODE"] + "' order by ROW_ID;";

        }
        if (tbe == desgna || tbm == desgna)
        {
            StrQrry = " SELECT [DESIGNATION],[EMP_NAME],[CLIENT_HQ],[NOP_HQ],[PRIMARY_VAL],[SECONDARY_VAL],[SnBPr] as [SnB %] FROM [EMPLOYEE_Incentive_TAB] where FS_Code ='" + Session["FSCODE"] + "' order by ROW_ID;";

        }
        if (desgna == "Admin")
        {
            StrQrry = " SELECT [DESIGNATION],[EMP_NAME],[CLIENT_HQ],[NOP_HQ],[PRIMARY_VAL],[SECONDARY_VAL],[SnBPr] as [SnB %] FROM [EMPLOYEE_Incentive_TAB] order by ROW_ID;";
        ...
        Response.Charset = "";
        string FileName = "PRI vs SEC " + Session["FSCODE"] + "_" + DateTime.Now + ".xls";
