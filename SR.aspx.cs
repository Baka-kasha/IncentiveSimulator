...
public partial class SR : System.Web.UI.Page
{
    ...
    private void ExportGridToExcel()
    {
        ...
        if (sm == desgna)
        {
            StrQrry = " SELECT [DESIGNATION],[EMP_NAME],[HQ],[TARGET_VAL],[PRIMARY_VAL],[SR][SALES_RETURN],[PrAfterSR][PRIMARY_VALUE_AFTER_DEDUCTING_SALES_RETURN],[ACH%] FROM [EMPLOYEE_Incentive_TAB] where SM ='" + Session["FSCODE"] + "' order by ROW_ID;";

        }
        if (rbm == desgna)
        {
            StrQrry = " SELECT [DESIGNATION],[EMP_NAME],[HQ],[TARGET_VAL],[PRIMARY_VAL],[SR][SALES_RETURN],[PrAfterSR][PRIMARY_VALUE_AFTER_DEDUCTING_SALES_RETURN],[ACH%] FROM [EMPLOYEE_Incentive_TAB] where RBM ='" + Session["FSCODE"] + "' order by ROW_ID;";

        }
        if (dsm == desgna)
        {
            StrQrry = " SELECT [DESIGNATION],[EMP_NAME],[HQ],[TARGET_VAL],[PRIMARY_VAL],[SR][SALES_RETURN],[PrAfterSR][PRIMARY_VALUE_AFTER_DEDUCTING_SALES_RETURN],[ACH%] FROM [EMPLOYEE_Incentive_TAB] where DSM ='" + Session["FSCODE"] + "' order by ROW_ID;";

        }
        if (tbe == desgna || tbm == desgna)
        {
            StrQrry = " SELECT [DESIGNATION],[EMP_NAME],[HQ],[TARGET_VAL],[PRIMARY_VAL],[SR][SALES_RETURN],[PrAfterSR][PRIMARY_VALUE_AFTER_DEDUCTING_SALES_RETURN],[ACH%] FROM [EMPLOYEE_Incentive_TAB] where FS_Code ='" + Session["FSCODE"] + "' order by ROW_ID;";

        }
        if (desgna == "Admin")
        {
            StrQrry = " SELECT [DESIGNATION],[EMP_NAME],[HQ],[TARGET_VAL],[PRIMARY_VAL],[SR][SALES_RETURN],[PrAfterSR][PRIMARY_VALUE_AFTER_DEDUCTING_SALES_RETURN],[ACH%] FROM [EMPLOYEE_Incentive_TAB] order by ROW_ID;";
        ...
        Response.Charset = "";
        string FileName = "SALES RETURN " + Session["FSCODE"] + "_" + DateTime.Now + ".xls";
        ...
        foreach (DataGridItem item in dg.Items)
        {
            ...
            {
                item.Cells[7].Attributes.Add("style", "mso-number-format:'0.00%'");
            }
    ...
    protected void Organo_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (double.TryParse(e.Row.Cells[10].Text, out percentageValue))
            {
            ...
            if (double.TryParse(e.Row.Cells[9].Text, out percentageValue1))
            {
                string formattedPercentage1 = percentageValue1.ToString("0.00");
                e.Row.Cells[9].Text = formattedPercentage1;
            }
            else
            {
                // Parsing failed, handle the case where the text cannot be parsed as a double
