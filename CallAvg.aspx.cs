...
public partial class CallAvg : System.Web.UI.Page
{
    ...
    private void ExportGridToExcel()
    {
        ...
        if (sm == desgna)
        {
            StrQrry = " SELECT  [DESIGNATION] ,[EMP_NAME],[CLIENT_HQ],[NOP_HQ],[CALLAVG]  FROM [EMPLOYEE_Incentive_TAB] where SM ='" + Session["FSCODE"] + "' order by ROW_ID;";

        }
        if (rbm == desgna)
        {
            StrQrry = " SELECT  [DESIGNATION] ,[EMP_NAME],[CLIENT_HQ],[NOP_HQ],[CALLAVG]  FROM [EMPLOYEE_Incentive_TAB] where RBM ='" + Session["FSCODE"] + "' order by ROW_ID;";

        }
        if (dsm == desgna)
        {
            StrQrry = " SELECT  [DESIGNATION] ,[EMP_NAME],[CLIENT_HQ],[NOP_HQ],[CALLAVG]  FROM [EMPLOYEE_Incentive_TAB] where DSM ='" + Session["FSCODE"] + "' order by ROW_ID;";

        }
        if (tbe == desgna || tbm == desgna)
        {
            StrQrry = " SELECT   [DESIGNATION] ,[EMP_NAME],[CLIENT_HQ],[NOP_HQ],[CALLAVG]  FROM [EMPLOYEE_Incentive_TAB] where  FS_Code ='" + Session["FSCODE"] + "' order by ROW_ID;";

        }
        if (desgna == "Admin")
        {
            StrQrry = " SELECT  [DESIGNATION] ,[EMP_NAME],[CLIENT_HQ],[NOP_HQ],[CALLAVG]  FROM [EMPLOYEE_Incentive_TAB] order by ROW_ID;";
        ...
        Response.Charset = "";
        string FileName = "CALL AVG " + Session["FSCODE"] + "_" + DateTime.Now + ".xls";
    ...
    public void binddropdown1()
    {
        ...
        if (sm == desgna)
        {
            ...
            DataTable dtDsStockist = new DataTable();
            string StrQrry = "";
            StrQrry = "Select distinct [ZONE] from [Organogram$] where where SM='" + Session["FSCODE"] + "'  ";
