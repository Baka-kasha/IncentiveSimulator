...
public partial class KPI2 : System.Web.UI.Page
{
    ...
    public void BindGrid()
    {
        ...
        if (sm == desgna)
        {
            ...
            string StrQrry = "";
            StrQrry = "";
            StrQrry = "	 SELECT * FROM [EMPLOYEE_Incentive_KPI2_TAB] where SM='" + Session["FSCODE"] + "' order by ROW_ID ";
            string conn = ConfigurationManager.ConnectionStrings["sqlcon"].ConnectionString;
        ...
        if (rbm == desgna || (rm == desgna && tea == "GT+MT") || (zbm == desgna && tea == "AX"))
        {
            ...
            string StrQrry = "";
            StrQrry = "";
            StrQrry = "	 SELECT * FROM [EMPLOYEE_Incentive_KPI2_TAB] where RBM='" + Session["FSCODE"] + "' order by ROW_ID ";
            string conn = ConfigurationManager.ConnectionStrings["sqlcon"].ConnectionString;
            SqlDataAdapter adpRpt = new SqlDataAdapter(StrQrry, conn);
            adpRpt.Fill(DsStockist, "DsStockist");
            adpRpt.Fill(dtDsStockist);
        ...
        if (dsm == desgna || asm == desgna || (rm == desgna && tea == "AX") || nkam == desgna)
        {
            ...
            string StrQrry = "";
            StrQrry = "";
            StrQrry = "	 SELECT * FROM [EMPLOYEE_Incentive_KPI2_TAB] where DSM='" + Session["FSCODE"] + "' order by ROW_ID ";
            string conn = ConfigurationManager.ConnectionStrings["sqlcon"].ConnectionString;
            SqlDataAdapter adpRpt = new SqlDataAdapter(StrQrry, conn);
            adpRpt.Fill(DsStockist, "DsStockist");
            adpRpt.Fill(dtDsStockist);
        ...
        if (tbe == desgna || tbm == desgna || tso == desgna || kam == desgna || kae == desgna)
        {
            ...
            string StrQrry = "";
            StrQrry = "";
            StrQrry = "	 SELECT * FROM [EMPLOYEE_Incentive_KPI2_TAB] where FS_Code ='" + Session["FSCODE"] + "'order by ROW_ID ";
            string conn = ConfigurationManager.ConnectionStrings["sqlcon"].ConnectionString;
            SqlDataAdapter adpRpt = new SqlDataAdapter(StrQrry, conn);
            adpRpt.Fill(DsStockist, "DsStockist");
            adpRpt.Fill(dtDsStockist);
        ...
        if (desgna == "Admin")
        {
            ...
            string StrQrry = "";
            StrQrry = "";
            StrQrry = "	 SELECT * FROM [EMPLOYEE_Incentive_KPI2_TAB] order by ROW_ID ";
            string conn = ConfigurationManager.ConnectionStrings["sqlcon"].ConnectionString;
            SqlDataAdapter adpRpt = new SqlDataAdapter(StrQrry, conn);
            adpRpt.Fill(DsStockist, "DsStockist");
            adpRpt.Fill(dtDsStockist);
    ...
    private void ExportGridToExcel()
    {
        ...
        if (sm == desgna)
        {
            StrQrry = " SELECT [DESIGNATION] ,[EMP_NAME],[CLIENT_HQ],[NOP_HQ],[E-CALL],CME,IQVIA,EXPIRY,AMOUNT FROM [EMPLOYEE_Incentive_KPI2_TAB] where SM ='" + Session["FSCODE"] + "' order by ROW_ID;";

        }
        if (rbm == desgna || (rm == desgna && tea == "GT+MT"))
        {
            StrQrry = " SELECT [DESIGNATION] ,[EMP_NAME],[CLIENT_HQ],[NOP_HQ],[E-CALL],CME,IQVIA,EXPIRY,AMOUNT FROM [EMPLOYEE_Incentive_KPI2_TAB] where RBM ='" + Session["FSCODE"] + "' order by ROW_ID;";

        }
        if (dsm == desgna || asm == desgna || (rm == desgna && tea == "AX") || nkam == desgna)
        {
            StrQrry = " SELECT [DESIGNATION] ,[EMP_NAME],[CLIENT_HQ],[NOP_HQ],[E-CALL],CME,IQVIA,EXPIRY,AMOUNT FROM [EMPLOYEE_Incentive_KPI2_TAB] where DSM ='" + Session["FSCODE"] + "' order by ROW_ID;";

        }
        if (tbe == desgna || tbm == desgna || tso == desgna || kam == desgna || kae == desgna)
        {
            StrQrry = " SELECT [DESIGNATION] ,[EMP_NAME],[CLIENT_HQ],[NOP_HQ],[E-CALL],CME,IQVIA,EXPIRY,AMOUNT FROM [EMPLOYEE_Incentive_KPI2_TAB] where FS_Code ='" + Session["FSCODE"] + "' order by ROW_ID;";

        }
        if (desgna == "Admin")
        {
            StrQrry = " SELECT [DESIGNATION] ,[EMP_NAME],[CLIENT_HQ],[NOP_HQ],[E-CALL],CME,IQVIA,EXPIRY,AMOUNT FROM [EMPLOYEE_Incentive_KPI2_TAB] order by ROW_ID;";
        ...
        Response.Charset = "";
        string FileName = "KPI 2 " + Session["FSCODE"] + "_" + DateTime.Now + ".xls";
    ...
    protected void Organo_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (e.Row.Cells[3].Text == rbm || e.Row.Cells[3].Text == sm)
            {
                ...
                {
                    SqlCommand cmd = new SqlCommand("SELECT * FROM [dbo].[KPI2$] where DESIGNATION = '" + Designation + "' and TEAM = '" + TEAM + "' ;", conn);
                    conn.Open();
                    ...
                    for (int i = 0; i < logintbl.Rows.Count; i += 3)
                    {
                        ...
                        if (TEAM == TEAM1)
                        {
                            ...
                            decimal VFPR1 = Convert.ToDecimal(VF);
                            VFPR1 = Math.Round(VFPR1, 2);

                            int VFPR = Convert.ToInt32(VFPR1);
                            ...
                            if (VFPR == value11)
                            {
                                ...
                                SqlCommand cmmd = new SqlCommand("[EMPLOYEE_Incentive_KPI2]", conn);
                                {

                                    string FSCODE = e.Row.Cells[5].Text;
                                    cmmd.CommandType = CommandType.StoredProcedure;
                                    cmmd.Parameters.AddWithValue("@AMOUNT", INCENTSP.ToString());
                                    cmmd.Parameters.AddWithValue("@CME", VF.ToString());
                                    cmmd.Parameters.AddWithValue("@FS_Code", FSCODE.ToString());
                                    cmmd.Parameters.AddWithValue("@IQVIA", IQVIA.ToString());
                                    cmmd.Parameters.AddWithValue("@ECALLS", ECALLS.ToString());
                        ...
                        else
                        {
                            ...
                            if (VFPR <= value11 && VFPR > value112)
                            {
                                ...
                                SqlCommand cmmd = new SqlCommand("[EMPLOYEE_Incentive_KPI2]", conn);
                                {
                                    ...
                                    cmmd.CommandType = CommandType.StoredProcedure;
                                    cmmd.Parameters.AddWithValue("@AMOUNT", INCENTSP.ToString());
                                    cmmd.Parameters.AddWithValue("@IQVIA", VF.ToString());
                                    cmmd.Parameters.AddWithValue("@FS_Code", FSCODE.ToString());
                                    cmmd.Parameters.AddWithValue("@CME", IQVIA.ToString());
                                    cmmd.Parameters.AddWithValue("@ECALLS", ECALLS.ToString());
                                    cmmd.Parameters.AddWithValue("@EXPIRY", EXP.ToString());
                                    // cmmd.Parameters.AddWithValue("@Datetime", Session["login"].ToString());
                                    //cmd.ExecuteNonQuery();
                    ...
                    for (int i = 0; i < logintbl.Rows.Count; i += 3)
                    {
                        ...
                        if (VFPR >= value11 && VFPR < value212)
                        {
                            ...
                            SqlCommand cmmd = new SqlCommand("[EMPLOYEE_Incentive_KPI2]", conn);
                            {
                                ...
                                cmmd.CommandType = CommandType.StoredProcedure;
                                cmmd.Parameters.AddWithValue("@AMOUNT", INCENTSP.ToString());
                                cmmd.Parameters.AddWithValue("@ECALLS", VF.ToString());
                                cmmd.Parameters.AddWithValue("@FS_Code", FSCODE.ToString());
                                cmmd.Parameters.AddWithValue("@IQVIA", IQVIA.ToString());
                                cmmd.Parameters.AddWithValue("@CME", ECALLS.ToString());
                                cmmd.Parameters.AddWithValue("@EXPIRY", EXP.ToString());
                                // cmmd.Parameters.AddWithValue("@Datetime", Session["login"].ToString());
                                //cmd.ExecuteNonQuery();
                                string strRsslt = cmmd.ExecuteNonQuery().ToString();
            ...
            if (tso == Designation || kae == Designation || asm == e.Row.Cells[3].Text || (rm == e.Row.Cells[3].Text && e.Row.Cells[2].Text == "GT+MT"))
            {
                ...
                using (SqlConnection conn = new SqlConnection(con1n))
                {
                    ...
                    for (int i = 0; i < logintbl.Rows.Count; i += 3)
                    {
                        ...
                        if (TEAM != TEAM1)
                        {
                            string VF1 = e.Row.Cells[10].Text;
                            VF1 = VF1.Replace("&nbsp;", "0");
                            decimal VFPR12 = Convert.ToDecimal(VF1);
                            VFPR12 = Math.Round(VFPR12, 2);

                            int VF = Convert.ToInt32(VFPR12);
                            decimal VFPR = Convert.ToDecimal(VF);
                           // VFPR = Math.Round(VFPR, 2);
            ...
            {
                string formattedPercentage = percentageValue1.ToString("0.00    ");
            ...
            // Attempt to parse the text value to a double
            if (double.TryParse(e.Row.Cells[12].Text, out percentageValue3))
            {
                string formattedPercentage = percentageValue3.ToString("P");
    ...
    protected void refersh_Click(object sender, EventArgs e)
    {
        ...
        if (sm == desgna)
        {
            ...
            string StrQrry = "";
            StrQrry = "";
            StrQrry = "	 SELECT * FROM [EMPLOYEE_Incentive_KPI2_TAB] where SM='" + Session["FSCODE"] + "' " + whreclase + " order by ROW_ID ";
            string conn = ConfigurationManager.ConnectionStrings["sqlcon"].ConnectionString;
            SqlDataAdapter adpRpt = new SqlDataAdapter(StrQrry, conn);
            adpRpt.Fill(DsStockist, "DsStockist");
            adpRpt.Fill(dtDsStockist);
        ...
        if (rbm == desgna)
        {
            ...
            string StrQrry = "";
            StrQrry = "";
            StrQrry = "	 SELECT * FROM [EMPLOYEE_Incentive_KPI2_TAB] where RBM='" + Session["FSCODE"] + "'  " + whreclase + " order by ROW_ID ";
            string conn = ConfigurationManager.ConnectionStrings["sqlcon"].ConnectionString;
            SqlDataAdapter adpRpt = new SqlDataAdapter(StrQrry, conn);
            adpRpt.Fill(DsStockist, "DsStockist");
            adpRpt.Fill(dtDsStockist);
        ...
        if (dsm == desgna)
        {
            ...
            string StrQrry = "";
            StrQrry = "";
            StrQrry = "	 SELECT * FROM [EMPLOYEE_Incentive_KPI2_TAB] where DSM='" + Session["FSCODE"] + "'  " + whreclase + " order by ROW_ID ";
            string conn = ConfigurationManager.ConnectionStrings["sqlcon"].ConnectionString;
            SqlDataAdapter adpRpt = new SqlDataAdapter(StrQrry, conn);
            adpRpt.Fill(DsStockist, "DsStockist");
            adpRpt.Fill(dtDsStockist);
        ...
        if (desgna == "Admin")
        {
            ...
            string StrQrry = "";
            StrQrry = "";
            StrQrry = "	 SELECT * FROM [EMPLOYEE_Incentive_KPI2_TAB] where " + whreclase + " order by ROW_ID ";
            string conn = ConfigurationManager.ConnectionStrings["sqlcon"].ConnectionString;
            SqlDataAdapter adpRpt = new SqlDataAdapter(StrQrry, conn);
            adpRpt.Fill(DsStockist, "DsStockist");
            adpRpt.Fill(dtDsStockist);
